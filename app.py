...
def db():
 DATA_DIR.mkdir(parents=True,exist_ok=True);c=sqlite3.connect(DB_PATH);c.row_factory=sqlite3.Row;c.execute('CREATE TABLE IF NOT EXISTS outbound_queue(id TEXT PRIMARY KEY,topic TEXT NOT NULL,payload TEXT NOT NULL,priority INTEGER NOT NULL DEFAULT 5,created_at TEXT NOT NULL,attempts INTEGER NOT NULL DEFAULT 0,last_error TEXT,delivered_at TEXT,target_system TEXT NOT NULL DEFAULT "UNG-ATLAS",receipt TEXT)');c.execute('CREATE TABLE IF NOT EXISTS local_events(id TEXT PRIMARY KEY,topic TEXT NOT NULL,payload TEXT NOT NULL,created_at TEXT NOT NULL)')
 for sql in ['ALTER TABLE outbound_queue ADD COLUMN target_system TEXT NOT NULL DEFAULT "UNG-ATLAS"','ALTER TABLE outbound_queue ADD COLUMN receipt TEXT']:
  try:c.execute(sql)
...
def envelope(row):return {'message_id':row['id'],'source_system':NODE_ID,'target_system':row['target_system'],'message_type':row['topic'],'payload':json.loads(row['payload']),'priority':row['priority'],'classification':'internal'}
async def relay_event(row):
 body=envelope(row)
 if NEXUS_URL:return await post_json(f'{NEXUS_URL}/v1/messages',body)
...
async def sync_once():
 ...
 for r in rows:
  try:
   receipt=await relay_event(r);accepted=bool(receipt.get('accepted',True))
   if not accepted:raise RuntimeError('transport_not_accepted')
   c.execute('UPDATE outbound_queue SET delivered_at=?,last_error=NULL,receipt=? WHERE id=?',(utcnow(),json.dumps(receipt,separators=(',',':'))[:4000],r['id']));n+=1
...
async def startup():db().close();app.state.worker=asyncio.create_task(worker());app.state.heartbeat=asyncio.create_task(heartbeat_worker(metrics,headers,connectivity,ATLAS_STATE))
...
def queue(limit:int=100):
 c=db();rows=c.execute('SELECT id,topic,target_system,priority,created_at,attempts,last_error,delivered_at,receipt FROM outbound_queue ORDER BY created_at DESC LIMIT ?',(max(1,min(limit,500)),)).fetchall();c.close();return {'items':[dict(r) for r in rows]}
