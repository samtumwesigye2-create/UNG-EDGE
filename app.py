...
async def sync_once():
    conn = db()
    rows = conn.execute("""SELECT * FROM outbound_queue
                           WHERE delivered_at IS NULL
                           ORDER BY priority ASC, created_at ASC LIMIT 50""").fetchall()
    delivered = 0
    for row in rows:
        try:
            await relay_event(row)
            conn.execute("UPDATE outbound_queue SET delivered_at=?, last_error=NULL WHERE id=?",
                         (utcnow(), row["id"]))
            delivered += 1
        except Exception as e:
            conn.execute("UPDATE outbound_queue SET attempts=attempts+1,last_error=? WHERE id=?",
                         (str(e)[:500], row["id"]))
        conn.commit()
    conn.close()
    return delivered
...
def health():
    conn = db()
    queued = conn.execute("SELECT COUNT(*) n FROM outbound_queue WHERE delivered_at IS NULL").fetchone()["n"]
    conn.close()
...
def status():
    conn = db()
    queued = conn.execute("SELECT COUNT(*) n FROM outbound_queue WHERE delivered_at IS NULL").fetchone()["n"]
    retrying = conn.execute("SELECT COUNT(*) n FROM outbound_queue WHERE delivered_at IS NULL AND attempts>0").fetchone()["n"]
    conn.close()
    disk = os.statvfs("/")
    return {
        "node_id": NODE_ID,
...
def ingest(event: EventIn):
    ...
    conn = db()
    conn.execute("INSERT INTO local_events(id,topic,payload,created_at) VALUES(?,?,?,?)",
                 (eid, event.topic, payload, now))
    conn.execute("INSERT INTO outbound_queue(id,topic,payload,priority,created_at) VALUES(?,?,?,?,?)",
                 (eid, event.topic, payload, event.priority, now))
    conn.commit()
    conn.close()
    return {"accepted": True, "event_id": eid, "queued": True}

@app.post("/v1/sync")
async def sync():
    return {"ok": True, "delivered": await sync_once()}

@app.get("/v1/queue")
def queue(limit: int = 100):
    limit = max(1, min(limit, 500))
    conn = db()
    rows = conn.execute("""SELECT id,topic,priority,created_at,attempts,last_error,delivered_at
                           FROM outbound_queue ORDER BY created_at DESC LIMIT ?""", (limit,)).fetchall()
    conn.close()
